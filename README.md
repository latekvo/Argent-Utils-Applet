...
mesh on macOS needs the source checkout on disk (`DIPLOMAT_SELF_REPO` if it
...
python3 -m diplomat_app.mesh --set tokens=out          # "this machine is out of tokens"
python3 -m diplomat_app.mesh --dispatch review --prompt "…"   # route a job
```
...
- **Update** - pull the checkout, rebuild, and relaunch in place. Shows how many
  ...
  hands over). Needs the source checkout on disk (`DIPLOMAT_SELF_REPO`).
...
> `DiplomatCoreSmoke` core self-test), so name the target: `swift run Diplomat`.
