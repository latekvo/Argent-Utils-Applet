"""Self-update: fast-forward the checkout, rebuild diplomat-core, relaunch.

The Linux applet runs straight out of its git checkout (the autostart .desktop
Exec points at ``linux/diplomat``), so "update" is three steps: fast-forward
the checkout to its upstream, re-run ``scripts/build-core.sh`` to refresh the
installed diplomat-core binary, and start the launcher again. Relaunching is all
...
def repo_root() -> Path:
    ...
    env = os.environ.get("DIPLOMAT_SELF_REPO")
    if env:
        return Path(env)
    # This file is <repo>/linux/diplomat_app/selfupdate.py, so parents[2] = <repo>.
...
def pull() -> str:
    ...
    if not _has_git_identity(root):
        ident = ["-c", "user.name=Diplomat updater",
                 "-c", "user.email=diplomat@localhost"]
...
def build_core() -> None:
    ...
    Rebuilds the static diplomat-core CLI and installs it to
    ``~/.local/share/diplomat/diplomat-core`` — the actual "reinstall" step;
