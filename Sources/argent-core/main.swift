...
// argent-core: a thin CLI over ArgentUtilsCore so the Linux (Qt6/PySide6) front-end
...
//
// Usage:
//   argent-core build-prompt      < config.json    # prints the assembled prompt
//
...
// a field is absent). Core assets are resolved via $ARGENT_UTILS_CORE (or the usual
// relative fallbacks) — the caller should point it at the repo's core/ directory.

func die(_ msg: String, _ code: Int32) -> Never {
    FileHandle.standardError.write(Data(("argent-core: " + msg + "\n").utf8))
    exit(code)
}
...
guard args.count >= 2, args[1] == "build-prompt" else {
    die("usage: argent-core build-prompt  (JSON config on stdin)", 1)
}
