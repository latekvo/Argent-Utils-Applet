...
/// yet installed, offers to install ArgentUtils as a login LaunchAgent (via the
...
enum Daemon {
    static let label = "com.ignacy.argent-utils"
    ...
    static func offerInstallIfInteractive() -> Bool {
        ...
        guard !isInstalled else {
            out("ArgentUtils: already installed as a login daemon — running this instance for now.\n")
            return false
        }
        out("""
    ...
    static func install() -> Bool {
        guard let script = installScriptPath() else {
            err("ArgentUtils: scripts/install-autostart.sh not found (run from the repo).\n")
            return false
        }
        out("Installing… (log: /tmp/argent-utils-install.log)\n")
        let proc = Process()
        proc.executableURL = URL(fileURLWithPath: "/bin/bash")
        // nohup + background so the orphaned installer keeps running after we exit.
        proc.arguments = ["-c", "nohup /bin/bash \(shq(script)) > /tmp/argent-utils-install.log 2>&1 &"]
        do { try proc.run() } catch {
            err("ArgentUtils: install failed to start: \(error)\n"); return false
        }
