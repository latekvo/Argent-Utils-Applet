...
/// The macOS bridge to a local Argent Mesh node — the counterpart of the Linux
...
///   - a *driver* that spawns the node (`python3 -m argent_utils.mesh --daemon`, run
...
enum MeshBridge {
    ...
    /// The node's state directory (`ARGENT_MESH_DIR` override, else `~/.argent/mesh`) —
    /// matches `argent_utils.mesh.identity.mesh_dir`.
    static var stateDir: URL {
        if let env = ProcessInfo.processInfo.environment["ARGENT_MESH_DIR"], !env.isEmpty {
    ...
    /// Optional pre-shared join token (`ARGENT_MESH_SECRET`), presented on every control
    /// session — mirrors `mesh.config.secret`. Empty (the default) = open mesh.
    static var secret: String { ProcessInfo.processInfo.environment["ARGENT_MESH_SECRET"] ?? "" }
    ...
    static func ensureRunning() -> String? {
        ...
        guard RepoPaths.checkoutPresent else {
            return "no Argent Utils checkout at \(RepoPaths.root.path) — set ARGENT_UTILS_SELF_REPO to point at it"
        }
        guard let python = resolvePython() else {
            return "python3 not found — install Python 3 (or set ARGENT_PYTHON) to run a mesh node"
        }
        let p = Process()
        p.executableURL = URL(fileURLWithPath: python)
        p.arguments = ["-m", "argent_utils.mesh", "--daemon"]
    ...
    static func resolvePython() -> String? {
        if let env = ProcessInfo.processInfo.environment["ARGENT_PYTHON"],
