...
/// launcher, macOS rebuilds `Diplomat.app` (via `scripts/build-app.sh`) and `open`s
...
enum SelfUpdate {
    ...
    /// Rebuild `Diplomat.app` from the (freshly pulled) checkout via
    ...
    static func relaunch() throws {
        let app = root.appendingPathComponent("Diplomat.app")
        guard FileManager.default.fileExists(atPath: app.path) else {
            throw UpdateError(message: "Diplomat.app not found after rebuild")
        }
        let p = Process()
