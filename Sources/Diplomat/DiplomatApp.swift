...
final class AppDelegate: NSObject, NSApplicationDelegate {
    func applicationDidFinishLaunching(_ notification: Notification) {
        ...
        // DIPLOMAT_PRINT_PROMPT=mine|user (default mine).
        if let mode = env["DIPLOMAT_PRINT_PROMPT"] {
        ...
        // uses the same `com.ignacy.diplomat` defaults domain as the GUI.
        if env["DIPLOMAT_SETTINGS_DUMP"] == "1" {
        ...
        if let what = env["DIPLOMAT_RENDER"] {
        ...
        if env["DIPLOMAT_AUTOFIX_POLL"] == "1" {
...
enum QuitFlow {
    @MainActor static func confirm() {
        ...
        let alert = NSAlert()
        alert.messageText = "Quit Diplomat?"
...
enum SingleInstance {
    ...
    private static func log(_ msg: String) {
        FileHandle.standardError.write(Data("Diplomat singleton: \(msg)\n".utf8))
    }
}
